...
# 修改 XHS-Downloader 的 ROOT 路径（必须在导入 source 模块之前）
try:
    import source.module.static as static_module
    static_module.ROOT = DOWNLOAD_ROOT
    static_module.ROOT.mkdir(parents=True, exist_ok=True)
    print(f"[配置] 下载路径已设置为: {DOWNLOAD_ROOT}")
except ImportError as e:
    print(f"[警告] 无法修改 XHS-Downloader 路径: {e}")
