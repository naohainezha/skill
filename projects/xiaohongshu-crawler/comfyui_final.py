...
def convert_to_api_prompt(workflow):
    ...
    for node in workflow['nodes']:
        ...
        for i, input_def in enumerate(node_inputs):
            ...
            if has_link:
                # 有link：从link_map获取 [from_node, from_slot]
                link_id = input_def['link']
                if link_id in link_map:
                    inputs[input_name] = link_map[link_id]

            elif i < len(widgets):
                # 没有link：使用widgets_values（跳过Qwen3的source_path）
            ...
            if response.status_code == 200:
                ...
                if prompt_id in history:
                    ...
                    if status.get('status_str') == 'execution_error':
                        ...
                        if node_errors:
                            for nid, errors in node_errors.items():
                                print(f"    节点{nid}: {errors}")
                        return None
