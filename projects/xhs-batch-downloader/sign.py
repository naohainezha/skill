...
async def call_mnsv2(page: Page, sign_str: str, md5_str: str) -> str:
    ...
    try:
        # 首先检查 window.mnsv2 是否存在
        mnsv2_exists = await page.evaluate("() => typeof window.mnsv2 === 'function'")
        if not mnsv2_exists:
            print("[签名] window.mnsv2 函数不存在，尝试等待页面加载...")
            # 等待一段时间让页面完全加载
            await page.wait_for_timeout(3000)
            mnsv2_exists = await page.evaluate("() => typeof window.mnsv2 === 'function'")
            if not mnsv2_exists:
                print("[签名] window.mnsv2 仍然不存在")
                return ""
        
        # 调用 mnsv2
        result = await page.evaluate(f"window.mnsv2('{sign_str_escaped}', '{md5_str_escaped}')")
        if result:
            print(f"[签名] mnsv2 调用成功")
            return result
        else:
            print("[签名] mnsv2 返回空结果")
            return ""
    except Exception as e:
        print(f"[签名] 调用mnsv2失败: {e}")
        return ""
